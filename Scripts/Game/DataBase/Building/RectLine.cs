...
namespace Game.DataBase
{
    ...
    public class RectLine : MonoBehaviour
    {
        ...
        public void UpdateCorners(Vector2 start, Vector2 end, Vector2 localWorkflowStart, Vector2 localWorkflowEnd)
        {
            ...
            StartPoint.LocalCoordinates = new(Mathf.RoundToInt(start.x), Mathf.RoundToInt(start.y));
            EndPoint.LocalCoordinates = new(Mathf.RoundToInt(end.x), Mathf.RoundToInt(end.y));
            StartPoint.LocalWorfklowCoordinates = new(Mathf.RoundToInt(localWorkflowStart.x), Mathf.RoundToInt(localWorkflowStart.y));
            EndPoint.LocalWorfklowCoordinates = new(Mathf.RoundToInt(localWorkflowEnd.x), Mathf.RoundToInt(localWorkflowEnd.y));
        }
