...
namespace Game.UI.Overlay.Computer.DesignApp
{
    ...
    internal partial class BlueprintEditorCreator
    {
        ...
        public UnityAction<BlueprintPlacerBase> OnPlacerAdded;
        public UnityAction OnAnyPlacerRemoved;
        public UnityAction<BlueprintPlacerBase> OnPlacerRemoved;
        ...
        #region methods
        /// <summary>
        /// resourcesIdCount will be cleared and filled with required
        /// </summary>
        /// <param name="resourcesIdCount"></param>
        public void GetAllResourcesRequirements(Dictionary<ConstructionResourceInfo, int> resourcesIdCount)
        {
            resourcesIdCount.Clear();
            int additionalF2Count = BlueprintEditor.Instance.CurrentData.BuildingData.Floor2AdditionalCount;
            ...
            while (true)
            {
                ...
                int countScale = 1;
                if (startFloor == BuildingFloor.F2_FlooringRoof || startFloor == BuildingFloor.F2)
                {
                    countScale += additionalF2Count;
                }
                foreach (ObjectPool<BlueprintPlacerBase> pool in floor.ResourcesPool.Values)
                {
                    ...
                    if (count == 0) continue;
                    ConstructionResourceInfo resInfo = ((BlueprintResourcePlacer)pool.OriginalPrefab).Element.ResourceInfo;
                    int scaledCount = count * countScale;
                    if (!resourcesIdCount.TryAdd(resInfo, scaledCount))
                    {
                        resourcesIdCount[resInfo] += scaledCount;
                    }
                }
                if (maxFloor == startFloor) break;
