...
namespace Game.UI.Overlay.Computer.DesignApp
{
    internal partial class BlueprintEditorCreator
    {
        ...
        public class Floor
        {
            ...
            /// </summary>
            public BlueprintZone SpawnZone(Vector3 localPosition, List<Vector2> textureCoordinates, PlacementType placement, bool isSerializable, bool isTestCreated)
            {
                ...
                if (placement == PlacementType.Good || placement == PlacementType.Neutral)
                {
                    if (!hasGreenZones)
                    {
            ...
            /// Spawns zone with fully exported parameters (non placeable)
            /// </summary>
            public BlueprintZone SpawnZone(BlueprintZoneInfo data, bool isSerializable, bool isTestCreated) => SpawnZone(data.LocalPosition, data.TexturePoints.ToList(), data.Placement, isSerializable, isTestCreated);

            private void SpawnNewZones(IReadOnlyList<BlueprintZoneInfo> zones, bool isSerializable, bool isTestCreated)
            {
            ...
            public void GenerateZonesFor(Floor toFloor, out BlueprintZone oneGreenZone)
            {
                ...
                BuildingFloor currentFloor = FloorIndex;
                if (currentFloor == BuildingFloor.F1_Flooring || currentFloor == BuildingFloor.F2_FlooringRoof || currentFloor == BuildingFloor.F3_Roof)
                {
                    bool mustSpawnGreenZone = true;
                    if (!isOwnFloor)
                    {
                        //copy red zones (and green zone from F2F)
                        foreach (StaticPoolableObject obj in ZonesPool.Objects)
                        {
                            BlueprintZone zone = (BlueprintZone)obj;
                            if (!zone.IsUsing) continue;
                            if (zone.Placement == PlacementType.Neutral)
                            {
                                /*zone.SetTextureCoordinatesTo(_GenerateZonesFor_texutreCoords);
                                toFloor.SpawnZone(zone.Transform.localPosition, _GenerateZonesFor_texutreCoords, PlacementType.Good, false, false);*/
                                continue;
                            }
                            if (zone.Placement == PlacementType.Good)
                            {
                                if (currentFloor == BuildingFloor.F2_FlooringRoof && !zone.IsTestCreated && mustSpawnGreenZone)
                                {
                                    zone.SetTextureCoordinatesTo(_GenerateZonesFor_texutreCoords);
                                    oneGreenZone = toFloor.SpawnZone(zone.Transform.localPosition, _GenerateZonesFor_texutreCoords, PlacementType.Good, false, false);
                                    mustSpawnGreenZone = false;
                                }
                            ...
                            zone.SetTextureCoordinatesTo(_GenerateZonesFor_texutreCoords);
                            toFloor.SpawnZone(zone.Transform.localPosition, _GenerateZonesFor_texutreCoords, PlacementType.Bad, false, false);
                        }
                        //add roof red zones
                        foreach (ObjectPool<BlueprintPlacerBase> pool in ResourcesPool.Values)
                        {
                            ...
                            ConstructionResourceInfo resInfo = resourceOriginal.Element.ResourceInfo;
                            ConstructionType originalType = resInfo.ConstructionType;
                            if (originalType == ConstructionType.Roof)
                            {
                                SpawnZoneForResources(toFloor, pool, PlacementType.Bad);
                                continue;
                            }
                        }
                    }
                    if (mustSpawnGreenZone)
                ...
                bool canSpawnStairsZone = currentFloor == BuildingFloor.F1;
                bool canSpawnNeutralZone = ResourcesPool.Values.Exists(x =>
                {
                    BlueprintResourcePlacer r = ((BlueprintResourcePlacer)x.OriginalPrefab);
                    return r.Element.ResourceInfo.ConstructionSubtype == ConstructionSubtype.Staircase;
                }, out _) && currentFloor == BuildingFloor.F1 && BlueprintEditor.Instance.CurrentData.BlueprintInfo.BuildingInfo.MaxFloor == BuildingFloor.F3_Roof;
                //spawn zone for objects
                foreach (ObjectPool<BlueprintPlacerBase> pool in ResourcesPool.Values)
                {
                    BlueprintResourcePlacer resourceOriginal = (BlueprintResourcePlacer)pool.OriginalPrefab;
                    ConstructionResourceInfo resInfo = resourceOriginal.Element.ResourceInfo;
                    ConstructionSubtype originalSubtype = resInfo.ConstructionSubtype;
                    ConstructionType originalType = resInfo.ConstructionType;
                    ConstructionLocation originalLocation = resInfo.ConstructionLocation;

                    if (canSpawnStairsZone && originalSubtype == ConstructionSubtype.Staircase)
                    {
                        SpawnZoneForResources(toFloor, pool, PlacementType.Bad);
                        continue;
                    }
                    if (canSpawnNeutralZone && originalType == ConstructionType.Wall && originalLocation == ConstructionLocation.Outside)
                    {
                        SpawnZoneForResources(toFloor, pool, PlacementType.Neutral);
                        continue;
                    }
                }
            }
            private void SpawnZoneForResources(Floor toFloor, ObjectPool<BlueprintPlacerBase> pool, PlacementType placement)
            {
                foreach (BlueprintPlacerBase placer in pool.Objects)
                {
                    ...
                    toFloor.SpawnZone(Vector3.zero, _GenerateZonesFor_corners, placement, false, false);
                }
            }
            ...
            private BlueprintZone TryCreateZoneByMostDistancedPoint(Floor toFloor, Vector2 distantCoordinates, BlueprintResourcePlacer mostDistancedPlacer, ConstructionLocation mostDistancedLoopLocation)
            {
                ...
                return toFloor.SpawnZone(Vector3.zero, _TryCreateZoneByMostDistancedPoint_textureCoords, PlacementType.Good, false, isTestCreated);
            }
            /// <summary>
            ...
            public BlueprintResourcePlacer SpawnResource(BlueprintResource element)
            {
                ...
                Context.OnPlacerAdded?.Invoke(placer);
                return placer;
            }
            ...
            public void DeserializeBlueprintObjects(BlueprintData currentData)
            {
                ...
                SpawnNewZones(currentData.BlueprintInfo.BlueprintZones, true, false);
            ...
            private void UpdateHasGreenZones()
            {
                foreach (StaticPoolableObject placer in ZonesPool.Objects)
                {
                    BlueprintZone zone = (BlueprintZone)placer;
                    if (!zone.IsUsing) continue;
                    if (zone.Placement == PlacementType.Good || zone.Placement == PlacementType.Neutral)
                    {
                        hasGreenZones = true;
                        return;
                    }
            ...
            private void RemovePlacerFast(BlueprintPlacerBase placer)
            {
                ...
                Context.OnPlacerRemoved?.Invoke(placer);
            }
