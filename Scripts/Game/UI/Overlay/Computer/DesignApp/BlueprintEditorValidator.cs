...
namespace Game.UI.Overlay.Computer.DesignApp
{
    internal static class BlueprintEditorValidator
    {
        ...
        public static bool FloorCheckAwait => floorCheckAwait;
        private static bool floorCheckAwait = false;
        #endregion fields & properties

        #region methods
        private static readonly Dictionary<ConstructionResourceInfo, int> insufficientResourcesIdCount = new();
        private static readonly Dictionary<ConstructionResourceInfo, int> tempResourcesIdCount = new();
        public static bool IsAllResourcesSufficient(BlueprintEditorCreator creator, out Dictionary<ConstructionResourceInfo, int> insufficientResourcesCount, out Dictionary<ConstructionResourceInfo, int> allResourcesCount)
        {
            ...
            foreach (var kv in allResourcesCount)
            {
                int curId = kv.Key.Id;
                int count = 0;
                if (warehouse.ConstructionResources.Items.Exists(x => x.Id == curId, out ResourceData resData))
                {
                    count = resData.Count;
                }
                int difference = count - kv.Value;
                if (difference < 0)
                {
                    insufficientResourcesCount.Add(kv.Key, Mathf.Abs(difference));
                }
            }
            return insufficientResourcesCount.Count == 0;
        }
        public static bool IsTimeForBuildSufficient(Dictionary<ConstructionResourceInfo, int> allResources, out int requiredTime)
        {
            int totalObjects = 0;
            foreach (var kv in allResources)
            {
                totalObjects += kv.Value;
            }
            float difficulty = BlueprintEditor.Instance.CurrentData.BlueprintInfo.DifficultyScale;
            int objectsPerHour = 5;
            float requiredTimeFloat = (totalObjects / (float)objectsPerHour) * difficulty;
            if (requiredTimeFloat > 60) 
                requiredTimeFloat = Mathf.Pow(requiredTimeFloat, 0.95f);
            requiredTime = Mathf.RoundToInt(requiredTimeFloat);
            return requiredTime == 0 || GameData.Data.PlayerData.MonthData.FreeTime.CanDecreaseValue(requiredTime);
        }
        ...
        [Todo("Add room size checks")]
        public static bool IsRoomSizeCorrect(BlueprintRoom room)
        {
        ...
        {
            floorCheckAwait = true;
            OnCorrectCheck += delegate { floorCheckAwait = false; };
            OnBadCheck += delegate { floorCheckAwait = false; };
            ...
            foreach (ObjectPool<BlueprintPlacerBase> objectPool in floor.ResourcesPool.Values)
            {
                foreach (BlueprintPlacerBase element in objectPool.Objects)
                {
                    ...
                    if (!containOutsideDoor && resSubtype == ConstructionSubtype.Door && resLoc == ConstructionLocation.Outside)
                    {
                        containOutsideDoor = true;
                    }

                    if (!containStair && resSubtype == ConstructionSubtype.Staircase)
                    {
                        containStair = true;
                    }
                }
            ...
            int additionalF2Count = editor.CurrentData.BuildingData.Floor2AdditionalCount;
            ...
            if (((maxFloor == BuildingFloor.F3_Roof || maxFloor == BuildingFloor.F2) && currentFloor == BuildingFloor.F1) ||
                (currentFloor == BuildingFloor.F2 && additionalF2Count > 0))
            {
                if (!containStair)
                {
            ...
            if (currentFloor == BuildingFloor.F1)
            {
                if (!containOutsideDoor)
                {
                    OnBadCheck?.Invoke(LanguageLoader.GetTextByType(TextType.Game, 168));
                    yield break;
                }
            }
            else if (currentFloor == BuildingFloor.F2)
            {
                if (containOutsideDoor)
                {
                    OnBadCheck?.Invoke(LanguageLoader.GetTextByType(TextType.Game, 169));
                    yield break;
                }
            }
            ...
            if (currentFloor == BuildingFloor.F1 || currentFloor == BuildingFloor.F2)
            {
                ...
                List<BuildingRoom> allowedRooms = editor.CurrentData.BlueprintInfo.GetAllowedRooms(BlueprintEditor.Instance.Creator.CurrentBuildingFloor);
                foreach (StaticPoolableObject obj in floor.RoomsPool.Objects)
                {
                    if (!obj.IsUsing) continue;
                    BlueprintRoom room = (BlueprintRoom)obj;
                    if (room.Placement == PlacementType.Bad)
                    {
        ...
        {
            if (allowedRooms.Count == 0) return true;
            BuildingRoom firstRoomType = room.Marker == null ? BuildingRoom.Unknown : room.Marker.Marker.RoomType;
            if (!allowedRooms.Contains(firstRoomType))
            {
        ...
        public static bool FloorHasEmptyGreenZones(Floor floor)
        {
            ...
            foreach (StaticPoolableObject obj in floor.ZonesPool.Objects)
            {
                ...
                if (zone.Placement == PlacementType.Bad || zone.Placement == PlacementType.Neutral) continue;
                        ...
                        for (int i = 0; i < 4; ++i) //don't get center
                        {
                            ...
                            for (int j = 0; j < pointsScaleLength; ++j)
                            {
                                ...
                                for (int k = 0; k < collidedObjectsCount; ++k)
                                {
                                    ...
                                    {
                                        if (colZone.Placement == PlacementType.Bad || colZone.Placement == PlacementType.Neutral)
                                        {
        ...
        {
            BlueprintZone biggestZone = null;
            ...
            foreach (StaticPoolableObject obj in floor.ZonesPool.Objects)
            {
                ...
                BlueprintZone zone = (BlueprintZone)obj;
                if (zone.Placement == PlacementType.Good)
                {
                    //here may be bug
                    ...
                    float biggestZoneArea = rooms.CalculateRoomArea(biggestZone.LocalTexturePoints);
                    float zoneArea = rooms.CalculateRoomArea(zone.LocalTexturePoints);
                    if (zoneArea > biggestZoneArea)
                    {
                        biggestZone = zone;
                    }
                }
            }
            if (biggestZone == null) return false;
            return FloorHasOutsideObjectsNotInGreenZone(floor, biggestZone);
        }
        ...
        public static bool IsElementCollidedWithAny(BlueprintGraphic element, out HashSet<BlueprintPlacerBase> nearCollidedPlacers, out HashSet<BlueprintRoom> collidedRooms)
        {
            ...
            {
                if (IsElementCollidedWithOther(_IsElementCollidedWithAny_elementCorners, blueprintPlacer.BlueprintGraphic))
                    return true;
            }
            ...
            bool isElementResource = element is BlueprintResource res;
            bool isElementCornerOut = false;
            if (isElementResource)
            {
                isElementCornerOut = ((BlueprintResource)element).ResourceInfo.ConstructionSubtype == ConstructionSubtype.CornerOut;
            }
            foreach (BlueprintZone blueprintZone in collidedZones)
            {
                if (blueprintZone.Placement != PlacementType.Bad)
                {
                    if (!hasGreenZones || greenCornersCompleted) continue;
                    collidedAnyGreenZone = true;
                    int uncheckedCount = 0;
                    blueprintZone.SetTextureCoordinatesTo(polygonCornersBuffer1);
                    while (true)
                    {
                        int currentCount = _IsElementCollidedWithAny_elementCornersUnchecked.Count;
                        if (currentCount == 0) break;
                        Vector2 uncheckedCorner = _IsElementCollidedWithAny_elementCornersUnchecked[currentCount - 1];
                        if (!IsPointInsidePolygon(uncheckedCorner, polygonCornersBuffer1))
                        {
                ...
                {
                    //may be incorrect here, but currently there's no bugs
                    if (isElementCornerOut && IntersectsCountWithPolygon(element, blueprintZone) <= 2)
                    {
                        continue;
                    }
                    return true;
        ...
        public static int IntersectsCountWithPolygon(BlueprintGraphic element, PolygonBlueprintGraphic polygon)
        {
            element.SetEntrieLocalCollisionCorners(elementCornersBufferList1);
            polygon.SetTextureCoordinatesTo(polygonCornersBuffer1);
            float precision = BlueprintEditor.VECTOR_WORKFLOW_PRECISION;
            ...
            {
                Vector2 currentCorner = elementCornersBufferList1[i];
                if (CustomMath.IsPointInsidePolygon(polygonCornersBuffer1, currentCorner, precision))
                    totalIntersects++;
            }
